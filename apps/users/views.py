...
from MxOnline.settings import STATIC_URL, BASE_DIR, MEDIA_URL, MEDIA_ROOT
...
class TryView(View):
    def get(self,request):
        ...
        gong = UserProfile.objects.get(id=1)
        image_root = gong.image.name

        image_root = os.path.join(MEDIA_ROOT, image_root)

        doc.render({"name": gong.nick_name, "gong": InlineImage(doc, image_root,  width=Mm(30), height=Mm(30))})
